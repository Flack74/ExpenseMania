...
export function MonthlyChart({ expenses, monthDate, currency = "INR" }: { expenses: Expense[]; monthDate: Date; currency?: string }) {
  ...
  for (const e of expenses) {
    ...
    if (d.getFullYear() === year && d.getMonth() === month) {
      data[d.getDate() - 1].total += e.amount;
    }
  }
    ...
    <div className="h-52 w-full sm:h-56">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 8, right: 2, left: -24, bottom: 0 }}>
          ...
          />
          <Bar dataKey="total" fill="url(#barFill)" radius={[6, 6, 2, 2]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
