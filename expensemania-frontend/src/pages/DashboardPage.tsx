...
export default function DashboardPage() {
  ...
  function exportCsv() {
    ...
    exportExpensesCsv(expenses, `expensemania-${new Date().toISOString().slice(0, 10)}.csv`);
          ...
          <div className="flex min-h-16 items-center justify-between gap-3 px-3 py-3 sm:px-6 lg:px-8">
            <div className="flex min-w-0 items-center gap-3">
              <button
                ...
                className="grid h-10 w-10 place-items-center rounded-xl border border-border bg-card/70 text-muted-foreground lg:hidden"
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel>
                    <span className="block truncate">{user?.name || "ExpenseMania"}</span>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          >
            <button
              type="button"
              onClick={onClose}
              className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-xl border border-border bg-card/70"
              aria-label="Close navigation"
            >
              <X className="h-4 w-4" />
            </button>
...
function SidebarContent({ onLogout }: { onLogout?: () => void }) {
  const itemClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold transition ${
      isActive ? "bg-primary text-primary-foreground shadow-glow-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"
    }`;
  ...
  return (
    <div className="flex h-full flex-col">
      <Link to="/" className="mb-8 flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="truncate font-display text-xl font-bold tracking-normal">ExpenseMania</p>
      ...
      <div className="mt-8 rounded-[1.35rem] border border-border bg-card/60 p-4">
        <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
          <BarChart3 className="h-4 w-4 text-primary" /> Monthly focus
        </div>
      ...
      <div className="mt-auto space-y-3">
        ...
        {onLogout && (
          <button
            type="button"
            onClick={onLogout}
            className="flex w-full items-center gap-3 rounded-2xl border border-border bg-card/60 px-3 py-3 text-sm font-semibold text-muted-foreground hover:text-foreground"
          >
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center justify-between sm:mb-5">
        <div className="grid h-10 w-10 place-items-center rounded-2xl bg-primary/15 text-primary sm:h-11 sm:w-11">
